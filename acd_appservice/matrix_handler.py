...
class MatrixHandler:
        ...
        if Puppet.get_id_from_mxid(user_id):
            ...
            if puppet:
                try:
                    ...
                    # Si es así entonces result tendra contenido
                    # Si no, entonces genera una exepción
                    ...
                    self.log.warning(
                        f"I can't get an puppet for the room {room_id} in [MATRIX] :: {e}"
                    )

        puppet: Puppet = await Puppet.get_customer_room_puppet(room_id=room_id)

        if not puppet:
            self.log.warning(f"I can't get an puppet for the room {room_id}  in [DB]")
            return
    ...
    async def handle_notice(
        ...
        if puppet and not puppet.phone:
            ...
            response = await bridge_conector.mautrix_ping(user_id=puppet.custom_mxid)
            if (
