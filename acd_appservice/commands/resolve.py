...
async def resolve(evt: CommandEvent) -> Dict:
    ...
    try:
        if agent_id:
            await puppet.room_manager.user_leaves(room_id=room_id, user_id=agent_id)
        supervisors = evt.config["acd.supervisors_to_invite.invitees"]
        if supervisors:
            for supervisor_id in supervisors:
                await puppet.room_manager.user_leaves(room_id=room_id, user_id=supervisor_id)
